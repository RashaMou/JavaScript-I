...
const carInfo = (carNumber) => {
  const indexNumber = carNumber - 1; // gets the index number from the car number passed to the function
  const carDetails = inventory[indexNumber]; // extracts object at index number and stores it into variable carDetails
  const carArray = Object.values(carDetails); // returns array of carDetails values
  carArray.shift(); //removes first item from array (id)
  console.log(`Car ${carNumber} is a ${carArray[2]} ${carArray[0]} ${carArray[1]}.`);
}
...
// let lastCar = 0;                                                      // Not sure how to solve with this

let lastCar = () => {                     
  lastCarDetails = inventory.pop();                                     
  console.log(`${lastCarDetails.car_make} ${lastCarDetails.car_model}`); 
}
...
let carModels = [];                         // create empty array to hold values of car_model 

const makeCarModelArray = () => {
  for(i=0; i<inventory.length; i++) {         // iterate over inventory array by index
    carModels.push(inventory[i].car_model);   // add value of car_model in each inventory item at index [i] to carModels array
    carModels.sort();                         // sort carModels in ascending order
  }
  console.log(carModels);
}  
makeCarModelArray(inventory);

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
let carYears = [];                          // creates an empty array for car years

let makeCarYearArray = () => {
  for(i=0; i<inventory.length; i++) {       // iterate over inventory array by index
    carYears.push(inventory[i].car_year);   // adds value of car_year in each inventory item at index [i] to carYears array
  }
  console.log(carYears);
}

makeCarYearArray(inventory);
...
console.log(JSON.stringify(germanCars()));
